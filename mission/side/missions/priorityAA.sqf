...
//-------------------- 7. BRIEFING
	...
	priorityTargetText = "Flugabwehr Batterie"; publicVariable "priorityTargetText";
	"priorityMarker" setMarkerText "Prioritätsziel: Flugabwehr Batterie"; publicVariable "priorityMarker";
	_briefing = "<t align='center' size='2.2'>Prioritätsziel</t><br/><t size='1.5' color='#b60000'>Flugabwehr Batterie</t><br/>____________________<br/>OPFOR Kräfte nehmen eine Luftabwehr Batterie in Betrieb! Wir haben die Koordinaten mit Hilfe von Thermalbildern aufgeklärt und auf Ihrer Karte markiert.<br/><br/>Das ist ein Prioritätsziel!";
	GlobalHint = _briefing; hint parseText _briefing; publicVariable "GlobalHint";
	showNotification = ["NewPriorityTarget", "Zerstören Sie die Flugabwehrstellung!"]; publicVariable "showNotification";
...
while {_loopVar} do {
	...
	if (!alive priorityObj1) then {
		if (!alive priorityObj2) then {
			...
			_completeText = "<t align='center' size='2.2'>Prioritätsziel</t><br/><t size='1.5' color='#08b000'>NEUTRALISIERT</t><br/>____________________<br/>Gute Arbeit, Männer! Sie sollten Ihren Absprung so schnell wie möglich auf die Prioritätsziele ausführen.<br/><br/>Greifen Sie weiter das Hauptziel an. Wir werden Uns bei neuer Auftragslage melden.";
			GlobalHint = _completeText; hint parseText _completeText; publicVariable "GlobalHint";
			showNotification = ["CompletedPriorityTarget", "Flugabwehr Batterie NEUTRALISIERT"]; publicVariable "showNotification";
