    ...
    if(player != vehicle player) then
    {
					...
					
        {
            if((driver _vehicle == _x) || (gunner _vehicle == _x)) then
            {
				...
				{
					if !(typeof vehicle player in ["O_Heli_Transport_04_repair_F","O_Heli_Transport_04_medevac_F","O_Heli_Transport_04_fuel_F","O_Heli_Transport_04_covered_F","O_Heli_Transport_04_box_F","O_Heli_Transport_04_bench_F","O_Heli_Transport_04_ammo_F","O_Heli_Transport_04_F"]) then
					{
					...
					{
					_name = format ["<t size='0.85' color='#FFFFFF' shadow='1'>%1 %2</t> <img size='0.7' color='#CCFF66' shadow='1' image='a3\ui_f\data\IGUI\Cfg\Actions\getincommander_ca.paa'/><br/>", _name, (name _x)];
					};
				};
            }
            else
