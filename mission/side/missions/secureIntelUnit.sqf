...
//--------------------------------------------------------------------------- BRIEFING
	...
	sideMarkerText = "Information sicherstellen"; publicVariable "sideMarkerText";
	"sideMarker" setMarkerText "Neben Mission: Information sicherstellen"; publicVariable "sideMarker"; 	
	_briefing = "<t align='center'><t size='2.2'>Neue Neben Mission</t><br/><t size='1.5' color='#00B2EE'>Information sicherstellen</t><br/>____________________<br/>Wir haben von Einheimischen erfahren, dass sensible, strategische Informationen den Besitzer wechseln sollen. Das ist eine gute Gelegenheit!<br/><br/>Wir haben die Position der Übergabe auf Ihrer Karte markiert. Verlegen Sie dorthin und stellen Sie die Informationen sicher. Diese sollten sich entweder in einem der Fahrzeuge oder an einer der Personen befinden.</t>";
	GlobalHint = _briefing; hint parseText GlobalHint; publicVariable "GlobalHint";
	showNotification = ["NewSideMission", "Information sicherstellen"]; publicVariable "showNotification";
	sideMarkerText = "Information sicherstellen"; publicVariable "sideMarkerText";
...
while { sideMissionUp } do {
	...
	if (NOT_ESCAPING) then {
		...
		if (_intelObj call BIS_fnc_enemyDetected) then {
			...
			hqSideChat = "Sie wurden aufgeklärt und das Zielobjekt versucht mit den gesuchten Informationen zu fliehen!"; publicVariable "hqSideChat"; [WEST,"HQ"] sideChat hqSideChat;
	...
	if (HE_ESCAPED) exitWith {
			...
			hqSideChat = "Das Zielobjekt ist entkommen! Mission FEHLGESCHLAGEN!"; publicVariable "hqSideChat"; [WEST,"HQ"] sideChat hqSideChat;
			sideMissionUp = false; publicVariable "sideMissionUp";
			[] spawn QS_fnc_SMhintFAIL;
			{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["sideMarker", "sideCircle"]; publicVariable "sideMarker";
	...
	if (SM_SUCCESS) exitWith {
		...
		hqSideChat = "Die Daten konnten sichergestellt werden, Mission abgeschlossen! Die Daten werden zwecks Analyse an das Hauptquartier geschickt."; publicVariable "hqSideChat"; [WEST,"HQ"] sideChat hqSideChat;
		sideMissionUp = false; publicVariable "sideMissionUp";
